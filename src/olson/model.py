...
class Encoder(nn.Module):
    def __init__(self, latent_size, env):
        ...
        if(env == "highway"):
            self.hidden_units = 2 * 14 * 256
        elif(env == "roundabout"):
            self.hidden_units = 12 * 12 * 256
        elif(env == "pong"):
            self.hidden_units = 2 * 2 * 256
...
class Generator(nn.Module):
    def __init__(self, z_dim, action_size, env):
        ...
        self.deconv2 = nn.ConvTranspose2d(512 + action_size, 256, 4, stride=2, padding=(1,1)) 
        self.batch2 = nn.BatchNorm2d(256)
        self.deconv3 = nn.ConvTranspose2d(256 + action_size, 128, 4, stride=2, padding=(1,1)) 
        self.batch3 = nn.BatchNorm2d(128)
        self.deconv4 = nn.ConvTranspose2d(128 + action_size , 128, 4, stride=2, padding=(1,1)) 
        self.batch4 = nn.BatchNorm2d(128)
        self.deconv5 = nn.ConvTranspose2d(128 + action_size , 64, 4, stride=2, padding=(1,1))
        self.batch5 = nn.BatchNorm2d(64)
        if(env == "highway"):
            self.deconv6 = nn.ConvTranspose2d(64 + action_size , channels, (4,4), stride=(2,2), padding=(1,1))
            self.batch6 = nn.BatchNorm2d(64) #128*128
            self.deconv7 = nn.ConvTranspose2d(channels + action_size , channels, (1,4), stride=(1,2), padding=(0,1))#128*256
            self.batch7 = nn.BatchNorm2d(64)
            self.deconv8 = nn.ConvTranspose2d(channels + action_size , channels, (1,4), stride=(1,2), padding=(0,1))#128*512
            self.batch8 = nn.BatchNorm2d(64)
        elif(env == "roundabout"):
            self.deconv6 = nn.ConvTranspose2d(64 + action_size , channels, (4,4), stride=(2,2), padding=(1,1))
            self.batch6 = nn.BatchNorm2d(64) #128*128
            self.deconv7 = nn.ConvTranspose2d(channels + action_size , channels, (2,2), stride=(2,2), padding=(16,16))#224*224
            self.batch7 = nn.BatchNorm2d(64)
            self.deconv8 = nn.ConvTranspose2d(channels + action_size , channels, (4,4), stride=(2,2), padding=(1,1))#448*448
            self.batch8 = nn.BatchNorm2d(64)
        elif(env == "pong"):
            self.deconv6 = nn.ConvTranspose2d(64 + action_size , channels, (2,2), stride=(1,1), padding=(1,1))
            self.batch6 = nn.BatchNorm2d(64) #128*128
            self.deconv7 = nn.ConvTranspose2d(channels + action_size , channels, (2,2), stride=(2,2), padding=(20,20))#128*256
            self.batch7 = nn.BatchNorm2d(64)
            self.deconv8 = nn.ConvTranspose2d(channels + action_size , channels, (1,1), stride=(1,1), padding=(1,1))#128*512
            self.batch8 = nn.BatchNorm2d(64)
    ...
    def forward(self, x, y):
        ...
        x = F.relu(self.deconv6(catv(x,y)))
        x = F.relu(self.deconv7(catv(x,y)))
        x = self.deconv8(catv(x,y))
