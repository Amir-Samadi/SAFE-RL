...
def generate_counterfactual(z_n, desired_action, agent, P, args, speed = .01, MAX_ITERS=5000, verbose=False):
    target_action = Variable(torch.LongTensor([desired_action])).to(args.device)

    z_n_value = z_n.data.cpu().numpy()
    #import pdb; pdb.set_trace()
    i = 0
    while True:
        z_n = Variable(torch.FloatTensor(z_n_value).to(args.device), requires_grad=True)
        z_n = model.norm(z_n)
        #import pdb; pdb.set_trace()
        # logit = agent.pi(P(z_n))

        if args.agent_algo in ["DQN"]:
            logit = agent.policy.q_net.q_net(z_n)
        elif args.agent_algo in ["PPO", "A2C"]:
            logit = agent.policy.action_net(z_n)
        ...
        target_action_rep = target_action.repeat(logp_cf.size(0))
        loss = F.nll_loss(logp_cf, target_action_rep)
